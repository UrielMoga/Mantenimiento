<?php
	...
	$img  = mysql_result($res, 0, "archivo");
	$i = 4;
	require "menu.php";
?>
...
<html>
	...
	<body>
		...
		<table class="productos">
			...
			for($i=0;$i<3;$i++){ 
				$rp = rand("1", $nump);
				$sqlprod = "SELECT * FROM productos WHERE eliminado = 0 AND stock != 0 AND id = $rp";
				$resp  = mysql_query($sqlprod, $con);
			
				$id = mysql_result($resp, 0, "id");
				$nombre = mysql_result($resp, 0, "nombre");
				$costo = mysql_result($resp, 0, "costo");
				$img  = mysql_result($resp, 0, "archivo");
			?>
			<td class="prod">
